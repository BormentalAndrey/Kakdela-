 ...
 *
 * Три этапа:
 * 1. **Найди лишнее** — среди 4 предметов найти тот, что не подходит по категории
 * 2. **Продолжи ряд** — выбрать правильный следующий элемент в последовательности
 * 3. **Собери пазл** — собрать картинку из 4 частей (сетка 2×2)
 *
 * Персонаж Кузя помогает ребёнку и радуется успехам.
 *
 * @param level      Номер уровня (по умолчанию 4).
...
fun FindOddOneScreen(
    ...
    // Текущий этап: 0 = лишнее, 1 = ряд, 2 = пазл, 3 = завершено
